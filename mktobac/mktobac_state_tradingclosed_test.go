...
func Test_OrderBook_State_TradingClosed_Auction(t *testing.T) {
	...
	assert.AssertEqualT(t, 0, bk.BuyAuctionSize(), "buy size should be 0")
	assert.AssertEqualT(t, 0, bk.SellAuctionSize(), "sell size should be 0")
	fixmodel.ContainsExec(t, e1, "cli1", "id1", fixmodel.OrdStatusRejected, "reject", 0, 0, 1)
	fixmodel.ContainsExec(t, e2, "cli2", "id2", fixmodel.OrdStatusRejected, "reject", 0, 0, 1)
}
