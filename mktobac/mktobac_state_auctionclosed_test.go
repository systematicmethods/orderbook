...
func Test_OrderBook_State_AuctionClosed_Limit(t *testing.T) {
	...
	bk := NewOrderBook(ins, tradingevent.OrderBookEventTypeCloseAuction, clock.NewMock())
	...
	assert.AssertEqualT(t, 1, len(bk.BuyOrders()), "buy size should be 0")
	assert.AssertEqualT(t, 1, len(bk.SellOrders()), "sell size should be 0")
...
func Test_OrderBook_State_AuctionClosed_Market(t *testing.T) {
	...
	assert.AssertEqualT(t, 1, len(e1), "e1 empty")
	assert.AssertEqualT(t, 1, len(e2), "e2 empty")
	assert.AssertEqualT(t, 0, len(bk.BuyOrders()), "buy size should be 0")
	assert.AssertEqualT(t, 0, len(bk.SellOrders()), "sell size should be 0")
