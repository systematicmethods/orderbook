...
func Test_AddThreeOrders(m *testing.T) {
	...
	err = pt.Add(NewPriceTimeItem("orderid1", 1.2, time.Now(), "data"))
	printerror(err, m)
	err = pt.Add(NewPriceTimeItem("orderid2", 1.1, time.Now(), "data"))
	printerror(err, m)
	err = pt.Add(NewPriceTimeItem("orderid3", 1.1, time.Now(), "data"))
	printerror(err, m)
