...
func makeOrderBook_for_OrderBook_Auction(t *testing.T) OrderBook {
	...
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli1", "b1", SideBuy, 100, 1.05, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli2", "b2", SideBuy, 50, 1.05, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli3", "b3", SideBuy, 30, 1.05, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli4", "b4", SideBuy, 100, 1.04, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli5", "b5", SideBuy, 200, 1.03, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli6", "b6", SideBuy, 60, 1.02, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli7", "b7", SideBuy, 100, 1.02, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli8", "b8", SideBuy, 200, 0.99, aclock))

	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli11", "s1", SideSell, 300, 1.01, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli21", "s2", SideSell, 50, 1.01, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli31", "s3", SideSell, 100, 1.02, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli41", "s4", SideSell, 40, 1.03, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli51", "s5", SideSell, 250, 1.04, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli61", "s6", SideSell, 40, 1.05, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli71", "s7", SideSell, 200, 1.06, aclock))
...
func makeOrderBook_for_OrderBook_Auction_Extreme(t *testing.T) OrderBook {
	...
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli1", "b1", SideBuy, 100, 1.05/4, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli2", "b2", SideBuy, 50, 1.05, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli3", "b3", SideBuy, 30, 1.05*2, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli4", "b4", SideBuy, 100, 1.04, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli5", "b5", SideBuy, 200, 1.03*2, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli6", "b6", SideBuy, 60, 1.02, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli7", "b7", SideBuy, 100, 1.02*2, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli8", "b8", SideBuy, 200, 0.99, aclock))

	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli11", "s1", SideSell, 300, 1.01*4, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli21", "s2", SideSell, 50, 1.01, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli31", "s3", SideSell, 100, 1.02*2, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli41", "s4", SideSell, 40, 1.03, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli51", "s5", SideSell, 250, 1.04*2, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli61", "s6", SideSell, 40, 1.05, aclock))
	_, _ = bk.NewOrder(makeAuctionLimitOrder("cli71", "s7", SideSell, 200, 1.06*2, aclock))
