...
func Test_OrderBook_State_OpenOrderEntry_Trading_Limit_SamePrice(t *testing.T) {
	...
	e11, _ := bk.NewOrder(makeLimitOrder("cli1", "id2", SideBuy, 100, 1.01, aclock))
	...
	containsExec(t, e3, "cli1", "id1", OrdStatusFilled, "new", 100, 1.01, 1)
	containsExec(t, e3, "cli1", "id2", OrdStatusPartiallyFilled, "part", 1, 1.01, 1)
	containsExec(t, e3, "cli2", "id2", OrdStatusPartiallyFilled, "part", 100, 1.01, 1)
	containsExec(t, e3, "cli2", "id2", OrdStatusFilled, "fill", 1, 1.01, 1)
}

func Test_OrderBook_State_OpenOrderEntry_Trading_Limit_SellLessThanBuy(t *testing.T) {
	ins := instrument.NewInstrument(inst, "ABV Investments")
	aclock := makeMockClock(12, 34, 0)
	bk := NewOrderBook(ins, OrderBookEventTypeOpenOrderEntry, aclock)
	assert.AssertEqualT(t, *bk.Instrument(), ins, "instrument same")

	e1, _ := bk.NewOrder(makeLimitOrder("cli1", "id1", SideBuy, 100, 1.01, aclock))
	aclock.Add(time.Millisecond)
	e2, _ := bk.NewOrder(makeLimitOrder("cli2", "id2", SideSell, 101, 1.00, aclock))
	aclock.Add(time.Millisecond)
	eb2, _ := bk.NewOrder(makeLimitOrder("cli1", "id3", SideBuy, 100, 1.01, aclock))
	aclock.Add(time.Millisecond)
	eb3, _ := bk.NewOrder(makeLimitOrder("cli1", "id4", SideBuy, 100, 1.01, aclock))
	...
	containsExec(t, e3, "cli1", "id1", OrdStatusFilled, "new", 100, 1.01, 1)
	containsExec(t, e3, "cli2", "id2", OrdStatusPartiallyFilled, "part", 100, 1.01, 1)
	containsExec(t, e3, "cli1", "id3", OrdStatusPartiallyFilled, "part", 1, 1.00, 1)
	containsExec(t, e3, "cli2", "id2", OrdStatusFilled, "fill", 1, 1.00, 1)
}
