...
func Test_OrderBook_Auction_MinOrder(t *testing.T) {
	bk := newOrderBook_for_OrderBook_Auction(t)
...
func Test_OrderBook_Auction_FillOrdersWithState(t *testing.T) {
	bk := newOrderBook_for_OrderBook_Auction(t)
	//printOrders(bk.bk.auctionBookOrders())
	//buyorders := bk.bk.auctionBookOrders().buyOrders.Orders()
	//sellorders := bk.bk.auctionBookOrders().sellOrders.Orders()
	state := newAuctionCloseCalculator()
...
`
	expected := fixmodel.LoadExecCSV(csv)

	for _, ex := range expected {
		fixmodel.ContainsExecCSV(t, execs, ex, "execs")
	}
...
func Test_OrderBook_Auction_FillOrdersWithStateExterme(t *testing.T) {
	bk := newOrderBook_for_OrderBook_Auction_Extreme(t)
...
b8	0.99	200
*/
func newOrderBook_for_OrderBook_Auction(t *testing.T) *auction {
	...
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli1", "b1", fixmodel.SideBuy, 100, 1.05, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli2", "b2", fixmodel.SideBuy, 50, 1.05, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli3", "b3", fixmodel.SideBuy, 30, 1.05, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli4", "b4", fixmodel.SideBuy, 100, 1.04, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli5", "b5", fixmodel.SideBuy, 200, 1.03, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli6", "b6", fixmodel.SideBuy, 60, 1.02, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli7", "b7", fixmodel.SideBuy, 100, 1.02, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli8", "b8", fixmodel.SideBuy, 200, 0.99, aclock))

	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli11", "s1", fixmodel.SideSell, 300, 1.01, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli21", "s2", fixmodel.SideSell, 50, 1.01, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli31", "s3", fixmodel.SideSell, 100, 1.02, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli41", "s4", fixmodel.SideSell, 40, 1.03, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli51", "s5", fixmodel.SideSell, 250, 1.04, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli61", "s6", fixmodel.SideSell, 40, 1.05, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli71", "s7", fixmodel.SideSell, 200, 1.06, aclock))

	assert.AssertEqualT(t, 8, bk.BuySize(), "buy orders")
	assert.AssertEqualT(t, 7, bk.SellSize(), "sell orders")

	return bk
}

func newOrderBook_for_OrderBook_Auction_Extreme(t *testing.T) *auction {
	ins := instrument.NewInstrument(inst, "ABV Investments")
	bk := NewOrderBook(ins, tradingevent.OrderBookEventTypeOpenAuction, clock.NewMock())
	aclock := test.NewMockClock(12, 34, 0)

	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli1", "b1", fixmodel.SideBuy, 100, 1.05/4, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli2", "b2", fixmodel.SideBuy, 50, 1.05, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli3", "b3", fixmodel.SideBuy, 30, 1.05*2, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli4", "b4", fixmodel.SideBuy, 100, 1.04, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli5", "b5", fixmodel.SideBuy, 200, 1.03*2, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli6", "b6", fixmodel.SideBuy, 60, 1.02, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli7", "b7", fixmodel.SideBuy, 100, 1.02*2, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli8", "b8", fixmodel.SideBuy, 200, 0.99, aclock))

	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli11", "s1", fixmodel.SideSell, 300, 1.01*4, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli21", "s2", fixmodel.SideSell, 50, 1.01, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli31", "s3", fixmodel.SideSell, 100, 1.02*2, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli41", "s4", fixmodel.SideSell, 40, 1.03, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli51", "s5", fixmodel.SideSell, 250, 1.04*2, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli61", "s6", fixmodel.SideSell, 40, 1.05, aclock))
	_, _ = bk.NewOrder(NewAuctionLimitOrder(inst, "cli71", "s7", fixmodel.SideSell, 200, 1.06*2, aclock))
